...
export const AuthProvider = ({ children }) => {
  ...
  const logout = () => {
    ...
    // Efface le stockage local (clé actuelle + quelques clés possibles legacy)
    localStorage.removeItem('userToken');
    localStorage.removeItem('user');

    // ⚠️ Sécurisation légère (ne touche pas le reste du storage)
    localStorage.removeItem('token');
    localStorage.removeItem('authToken');
    localStorage.removeItem('jwt');

    // ✅ Redirige vers la route /login (définie dans App.jsx)
    navigate('/login');
  };
